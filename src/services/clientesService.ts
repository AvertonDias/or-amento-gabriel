...
const getClientesCollection = (userId: string) => {
  return collection(db, 'empresa', userId, 'clientes');
};
...
export const addCliente = (userId: string, cliente: Omit<ClienteData, 'id' | 'userId'>): Promise<string> => {
  const clientesCollection = getClientesCollection(userId);
...
export const updateCliente = (userId: string, clienteId: string, cliente: Partial<Omit<ClienteData, 'id' | 'userId'>>) => {
  const clienteDoc = doc(db, 'empresa', userId, 'clientes', clienteId);
